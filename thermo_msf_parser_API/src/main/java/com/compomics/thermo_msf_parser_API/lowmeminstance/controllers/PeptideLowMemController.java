...
public class PeptideLowMemController extends Observable implements PeptideInterface {
    ...
    public List<PeptideLowMem> getPeptidesForProtein(ProteinLowMem protein,MsfFile msfFile) {
        ...
        try {
            ...
            if (msfFile.getVersion() == MsfVersion.VERSION1_2) {
                ResultSet rs = stat.executeQuery("select p.PeptideID,ConfidenceLevel,Sequence,TotalIonsCount,MatchedIonsCount,Annotation,ProcessingNodeNumber, s.*, FileID as file from spectrumheaders as s, masspeaks, Peptides as p,Spectra as sp,(select PeptideID as ID from PeptidesProteins where ProteinID =" + protein.getProteinID() + ") as pepid where pepid.ID = p.PeptideID and masspeaks.masspeakid = s.masspeakid and s.SpectrumID = P.SpectrumID and sp.UniqueSpectrumID = s.UniqueSpectrumID order by p.PeptideID");
                while (rs.next()) {
                    PeptideLowMem lPeptide = new PeptideLowMem(rs.getInt("p.PeptideID"), rs.getInt("SpectrumID"), rs.getInt("ConfidenceLevel"), rs.getString("Sequence"), rs.getInt("TotalIonsCount"), rs.getInt("MatchedIonsCount"), rs.getString("Annotation"), rs.getInt("ProcessingNodeNumber"), msfFile.getAminoAcids());
    ...
    public void getPeptidesForProteinList(List<ProteinLowMem> proteinLowMemList, MsfFile msfFile) {
        try {
            ...
            listOfProteinIds = listOfProteinIds.replaceFirst(",", "");
            if (msfFile.getVersion() == MsfVersion.VERSION1_3) {
                PreparedStatement stat = msfFile.getConnection().prepareStatement("select p.PeptideID,ConfidenceLevel,Sequence,p.TotalIonsCount,p.MatchedIonsCount,p.Annotation,p.ProcessingNodeNumber,p.MissedCleavages,p.UniquePeptideSequenceID, s.*, FileID as file,pp.ProteinID from spectrumheaders as s, masspeaks as m, Peptides as p,Spectra as sp, (select PeptideID,ProteinID from peptidesProteins where PeptidesProteins.ProteinID in (" + listOfProteinIds + ")) as pp where m.masspeakid = s.masspeakid and s.SpectrumID = p.SpectrumID and s.UniqueSpectrumID = sp.UniqueSpectrumID and p.PeptideID = pp.PeptideID ");
                ResultSet rs = stat.executeQuery();
                while (rs.next()) {
                    PeptideLowMem lPeptide = new PeptideLowMem(rs.getInt("p.PeptideID"), rs.getInt("SpectrumID"), rs.getInt("ConfidenceLevel"), rs.getString("Sequence"), rs.getInt("p.TotalIonsCount"), rs.getInt("p.MatchedIonsCount"), rs.getString("p.Annotation"), rs.getInt("p.ProcessingNodeNumber"), msfFile.getAminoAcids());
